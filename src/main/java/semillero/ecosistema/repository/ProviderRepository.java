...
public interface ProviderRepository extends JpaRepository<ProviderEntity, String> {
    ...
    @Query("SELECT a FROM ProviderEntity a WHERE a.name LIKE %:nombre% and a.deleted = false and status = 'ACEPTADO'")
    ...
    @Query("SELECT a FROM ProviderEntity a WHERE a.deleted = false and a.category.nombre = :categoria and status = 'ACEPTADO'")
    ...
    @Query("SELECT p FROM ProviderEntity p WHERE (p.country.name = :pais and p.province.name = :provincia and p.city = :ciudad) and p.deleted = false and status = 'ACEPTADO'")
